...
function Unauthchat({ id }: ChatId) {
    ...
    const [firstincomemessage, setfirstincomemessage] = useState(true)

    const { messages, input, handleInputChange, handleSubmit, isLoading, error, setMessages, append, setInput } =
        useChat({
            api: "/api/chatunauth",
            body: {
            ...
            onResponse: response => {
                ...
                if (!response.ok) {
                    ...
                    const chatIdHeader = headers.get("X-Chat-ID");
                    if (chatIdHeader) {
                        setChatId(chatIdHeader); // Update chatId when you receive it in the headers
                    }
    ...
    const incomemessage = searchParams.get('message')


    useEffect(() => {
        if (incomemessage && firstincomemessage) {
            ...
            setfirstincomemessage(false)
            console.log(firstincomemessage)
            append(message).then((response) => {
                console.log('AI response:', response);
                // Clear the search parameters after processing the message
    ...
    }, [incomemessage, append, router, firstincomemessage]);
    ...
    return (
        <div className='w-full h-full mx-auto gap-2 max-w-sm sm:max-w-5xl'>
            ...
            <Card className='w-full h-full flex flex-col justify-end' >
                <CardContent >
                    <ScrollArea className=' w-full max-h-[calc(100vh-3.5rem-13rem)] min-h-full border-zinc-200  gap-4 p-3 overflow-y-auto focus-visible:ring-transparent scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch'>
                    ...
                    <form className='w-full' onSubmit={handleSubmit} onReset={clearchat} >
                        ...
                        <div className='flex flex-row justify-between w-full mt-2'>
                            <div className="flex items-center gap-1.5 ">
                                    ...
                                    <Select onValueChange={handleValueChange}>
                                        ...
                                        <SelectContent>
                                            <SelectGroup>
                                                ...
                                                <SelectItem value="gpt-4o-2024-05-13" disabled>GPT 4 O ⚡️</SelectItem>
                                ...
                                <Switch id="depth" onClick={toggledepth} disabled className="max-sm:hidden" />
                                <Label className="max-sm:hidden">Advance Search</Label>
                            </div>
                            <Button
